...
# Test authorization
curl -H "X-API-Key: read-only-key" -X POST http://localhost:8000/cache/clear  # Should return 403

# Test input validation
curl -H "X-API-Key: admin-key" -X POST http://localhost:8000/query \
  -d '{"metric_name": "'; DROP TABLE users; --"}' # Should return 400

# Test rate limiting
for i in {1..100}; do curl -H "X-API-Key: test-key" http://localhost:8000/metrics; done
